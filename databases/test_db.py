...
con = lite.connect('unittest.sqlite')
...
with con:
	...
	* Academics: anonymized_data/csv/PELL, GPA, Acad Standing, Degree (1.m,2.e,2.k,4.a).csv
	"""
	classestaken = []
	with open('testdata/Classes Taken.csv', mode = 'r', encoding='utf-8-sig') as classes1csv:
		reader = csv.reader(classes1csv)
		for row in reader:
			classestaken.append(row)

	demographics = []
	with open('testdata/Demographics 1i.csv', mode = 'r', encoding='utf-8-sig') as demographicscsv:
		reader = csv.reader(demographicscsv)
		for row in reader:
			demographics.append(row)

	finance = []
	with open('testdata/FAFSA, income, housing (1.e, 1.l, 2.n).csv', mode = 'r', encoding='utf-8-sig') as financecsv:
		reader = csv.reader(financecsv)
		for row in reader:
			finance.append(row)

	major = []
	with open('testdata/Major, school acceptances 1j, 1k .csv', mode = 'r', encoding='utf-8-sig') as majorcsv:
		reader = csv.reader(majorcsv)
		for row in reader:
			major.append(row)

	academics = []
	with open('testdata/PELL, GPA, Acad Standing, Degree (1.m,2.e,2.k,4.a).csv', mode = 'r', encoding='utf-8-sig') as academicscsv:
