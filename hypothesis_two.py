...
def print_results(gap_size, sample_size, pearson_coeff, p_value):
	...
	if p_value > 0.05:
		print("                            P-VALUE:  " + str(p_value) + " (INSIGNIFICANT)")
	else:
...
############## MAIN ##############
def two_main(classonedep, classonenum, classtwodep, classtwonum):

	student_list = create_table(classonenum, classonedep, classtwonum, classtwodep)
	...
	gap_two = get_class_group(2, student_list, reference_list)
	...
	gap_two_class1, gap_two_class2 = get_grades(gap_two)
	...
	gap_two_coeff, gap_two_p = get_pearson_coefficient(gap_two_class1, gap_two_class2)
	...
	print("############################## TIME CORRELATION RESULTS ##############################\n")
	print_results(1, len(gap_one_class1), gap_one_coeff, gap_one_p)
	print_results(2, len(gap_two_class1), gap_two_coeff, gap_two_p)
	...
	z, p = independent_corr(gap_one_coeff, gap_two_coeff, len(gap_one_class1), len(gap_two_class1))
	if p > 0.05:
		print("fisher z-transformation for (GAP = 1) and (GAP = 2): z = " + str(z) + ", p-val = " + str(p) + "\nINSIGNIFICANT DIFFERENCE")
	else:
	...
	#closing the connection to the database so more tests can be run
	global con
	con.close()

if __name__ == '__main__':
	two_main('CS', '0445', 'CS', '1501')
